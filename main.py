...
def mapper(json_value, data, parent=""):
    for key in json_value:
        if type(json_value[key]) is dict:
            data.append([f"{parent + key}", key, mapper_type(json_value[key]),
                        "", "Составной объект, имеет вложения"])
            mapper(json_value[key], data, parent=parent + key + ".")
        elif type(json_value[key]) is list:
            data.append([f"{parent + key}", key,
                        mapper_type(json_value[key]), "", "Массив"])
            try:
                mapper(json_value[key][0], data, parent=parent + key + "[*].")
            except:
                # TODO Странная реализация, подумать как сделать лучше
                if len(json_value[key]) > 0:
                    data.append([f"{parent + key}[*]", "-", mapper_type(
                        json_value[key][0]),
                        json_value[key][0],
                        "Элемент массива"])
                else:
                    data.append([f"{parent + key}[*]", "-",
                                "-", "-", "Пустой массив"])
        else:
            data.append([f"{parent + key}", key,
                        mapper_type(json_value[key]), json_value[key], ""])
...
df = pd.DataFrame(data_for_df,
                  columns=['Путь к переменной',
                           'Наименование переменной',
                           'Тип', 'Пример заполнения',
                           'Комментарий'])
